...
```r
ggplot(mpg, aes(cty, hwy)) + #Plot dataset mpg, x=cty, y=hwy.
  #geom_point()
  geom_jitter(alpha = 0.5, size = 1) + #geom_jitter is useful when we want to avoid overplotting.
...
gapminder %>%  
  #group_by(country) %>% 
  ggplot(aes(year, lifeExp, group = country, color = country == 'Rwanda')) + #instead of grouping a tibble, we use group inside the ggplot aesthetics. We also asign a specific color to Rwanda.
  geom_line(alpha = 0.2) + #alpha makes the lines more transparent. 
  scale_color_discrete("", labels = c("Other", "Rwanda")) #Names the legend of our plot.
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  scale_x_log10(labels = scales::comma_format()) + #by calling "scales" package, we use use comma_format function.
  facet_wrap(~ continent, scales = "free_y") #To display the plots of each continent. scale is different for each plot depending on its own data. 
```
...
```r
gapminder %>% 
  filter(continent != "Oceania") %>%  #We remove Oceania from our dataset.
  ggplot(aes(gdpPercap, lifeExp, size = pop, fill = continent)) + #The size of the points will change based on population. The color of the points will change based on the continent.
  facet_wrap(~ continent, nrow = 1) + #To display all of our plots into a single row. 
  geom_point(alpha = 0.5, shape = 21) + #The kind of figure we'd like to display our data, for this case are circles. 
  scale_x_log10(labels = scales::comma_format()) + #The x axis uses a comma for each label.
  scale_size_area() 
```
