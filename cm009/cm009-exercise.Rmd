...
Yes, each column is a variable. 

2. Widen the data so that we see the words spoken by each race, by putting race as its own column.

```{r}
(lotr_wide <- lotr %>% 
  pivot_wider(id_cols = c(-Race, -Words), #We remove columns "Race" and "Words".
              names_from = Race, #We take the name for our new columns from the "Race" column.
              values_from = Words)) #We take the values for our new columns from the "Words" column.
```

3. Re-lengthen the wide LOTR data from Question 2 above.

```{r}
lotr_wide %>%  #We will go back to where we started.
  pivot_longer(cols = c(-Film, -Gender), #We change our columns except for these ones. 
               names_to  = "Race", #We create a new column called Race.
               values_to = "Words") #We create another new column called Words.
```
...
```{r}
guest %>% 
  DT::datatable(rownames = FALSE) #A data table from the "guest" dataset.
```
...
```{r}
(guest_long <- guest %>% 
  pivot_longer(cols      = c(-party, -name), #We change everything but these two columns.
               names_to  = c(".value", "event"), #A new column called "event", its value will come from the columns that we set in cols. 
               names_sep = "_")) #The underscore that separates our variables. 
```
...
```{r}
guest_long %>% 
  separate(name, into = c("first", "last"), sep = " ") %>% #We type a space 
...
guest_long %>% 
  group_by(party) %>% 
  summarize(all_pending = all(attendance == "PENDING"))  #all will return logical values. 
```
...
```{r}
email %>% 
  separate_rows(guest, sep = ", ") #a comma and a space!
```
...
tibble(label = "Sample A", 
             x = rnorm(n), #rnorm is random normal
             y = x + rnorm(n)) 
```
...
experiment %>% complete(nesting(name, trt), rep) #complete gives all possible combinations.
                                                #nesting inside complete to pair name and treatment. 
```
